...
public class UeService {
    ...
    //get for all ues without any filter
    @GET
    ...
    //get for a ue on the name
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public List<Ue> getByName(@DefaultValue("") @QueryParam("name") String name){
        UeSearchVM s = new UeSearchVM();
        s.setName(name);
    ...
    @Path("{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public Ue find(@PathParam("id") Integer id){
        return ejb.find(id);
    }
    ...
    @Path("{from}/{to}")
    @Produces(MediaType.APPLICATION_JSON)
    public List<Ue> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to){
    ...
    @Path("{id}")
    @Consumes((MediaType.APPLICATION_JSON))
    public void edit(@PathParam("id") Integer id, Ue entity){
        ejb.edit(entity);
    }
